        ...
        def sum_accumulated_scores():
            for ctx_idx in range(len(self._ctxs)):
                for batch_sent_idxs, batch_scores, batch_masked_positions in zip(batch_sent_idxs_per_ctx[ctx_idx],
                    ...
                    if per_token:
                        # Slow; only use when necessary
                        for batch_sent_idx, batch_score, batch_masked_position in zip(batch_sent_idxs, batch_scores,
                            ...
                            scores_per_token[batch_sent_idx][
                                batch_masked_position.cpu().numpy().item()] = batch_score.cpu().numpy().item()
                    else:
                        # np.add.at(scores, batch_sent_idxs.asnumpy(), batch_scores.asnumpy())
